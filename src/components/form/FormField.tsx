...
export const FormField = ({ children, name, label, className, fluid = false }: Props) => {
  const ctx = useFormContext();
  const error = ctx.formState.errors[name]?.message as string;

  return (
    <div className="w-full">
      <label className="flex w-full flex-col gap-1 pl-[0.1rem] text-sm font-normal" htmlFor={name}>
        {label}
      </label>
      <div
        className={clsx(
      ...
      >
        {children}
        {error && <HiOutlineExclamationCircle className="h-6 w-6 stroke-1 text-red-500" />}
      </div>
